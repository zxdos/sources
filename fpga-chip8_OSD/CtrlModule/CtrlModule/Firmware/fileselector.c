...
static int romcount;
extern int vdcload; //Carga de charset de VDC

static void listroms();
static void selectrom(int row);
static void scrollroms(int row);
int (*loadfunction)(const char *filename, const char *extension); // Callback function
...
static void selectrom(int row)
{
	...
	if(p)
	{
		...
		if(loadfunction)
			(*loadfunction)(longfilename, p->Extension);
	}
}
