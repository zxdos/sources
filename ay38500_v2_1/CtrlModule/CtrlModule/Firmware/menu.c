...
int keys_p1[5] = { KEY_F5, KEY_F6, KEY_F7, 0x23, KEY_SCROLLLOCK}; // F5 - F6 - F7 - D -> Bloq.Despl - Used for control keys
int keys_p2[5] = { 0x43, 0x42, 0x3b, 0x4b, KEY_SCROLLLOCK}; // IKJL -> H (Default)
...
int Menu_Run()
{
	...
	if((TestKey(KEY_UPARROW)&2) || (TestKey(keys_p1[0])&2) || ((joy_pins & 0x10 )>0))
	{
	...
	if((TestKey(KEY_DOWNARROW)&2) || (TestKey(keys_p1[1])&2) || ((joy_pins & 0x08 )>0))
	{
